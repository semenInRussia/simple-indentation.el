...
(defun simple-indention-rules-get-basic-handler-of (keyword)
    ...
    (intern (s-concat "simple-indention-rules-handler-"
...
(defun simple-indention-rules-get-binary-handler-of (keyword)
    "Get basic handler of KEYWORD for `simple-indention-rules-make'."
    (intern (s-concat "simple-indention-rules-bin-handler-"
                      (s-chop-prefix ":" (format "%s" keyword)))))


(defun simple-indention-rules-begin-block-handler
    (_keyword rule _values right-args)
...
(defun simple-indention-rules-not-found-keyword-handler
    (keyword rule _values right-args)
...
(defun simple-indention-rules-special-handler-of-keyword-p
    (keyword pred-and-handler)
...
(defun simple-indention-rules-handler-on-current-or-previous-line (rule
                                                                    &rest _)
...
(defun simple-indention-rules-handler-on-current-or-previous-text-line (rule
                                                                         &rest
                                                                           _)
...
(defun simple-indention-rules-handler-on-current-or-previous-code-line (rule
                                                                         &rest
                                                                           _)
...
(defun simple-indention-rules-utils-or-run-func-before-indent-current-line-p
    (rule func)
    "Get func utils getting result of `indent-current-line-p' or run before FUNC.
...
Return new modified RULE.  Use RIGHT-ARGS for create second rule."
    (let* ((other-rule (apply #'simple-indention-rules-make right-args))
           (pred
            (lambda ()
                (or
                 (simple-indention-rules-indent-current-line-p rule)
...
(defun simple-indention-rules-make-line-has-keywords-p (keywords)
    ...
    (lambda () (simple-indention-utils-line-has-keywords-p keywords)))
...
(cl-defun simple-indention-rules-indent-line-with-sorted-rules
    (sorted-rules
     &key (each-line-before-indent-hook nil) (each-line-after-indent-hook nil))
