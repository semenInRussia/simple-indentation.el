;;; simple-indention.el --- Simple Define Indent's Funcs for Your Cool Major Modes -*- lexical-bindings: t -*-

;; Copyright (C) 2022 semenInRussia

;; Author: Semen Khramtsov <hrams205@gmail.com>
;; Version: 0.1
;; URL: https://github.com/semenInRussia/simple-indention.el
...
(require 'simple-indention-rules)
(require 'simple-indention-namespace)


(defgroup simple-indention nil
    "Package for easy define indention functions and vars."
    :group 'elisp)


(defcustom simple-indention-default-one-indent "    "
  "Default indention when define for major mode."
  :group 'simple-indention
  :type 'string)


(defvar simple-indention-decrement-indent-level-function nil
  "This is function, which deindent current line.
Please, don't touch, this is change automatically.")


(defvar simple-indention-increment-indent-level-function nil
  "This is function, which add indent to current line.
Please, don't touch, this is change automatically.")


(cl-defmacro simple-indention-define-for-major-mode
    (major-mode
     &optional namespace
     &key
       rules
       (one-indent simple-indention-default-one-indent)
...
CLEAR-OLD-INDENTION, CLEAR-EMPTY-LINES see docs."
    (let ((major-mode-name (symbol-name major-mode))
          (one-indent-of-mode `(eval
                                (simple-indention-namespace-from ,namespace
                                                                 one-indent))))
        `(progn
             ...
             (simple-indention-namespace-setq ,namespace indention-rules ,rules)
...
when indent some lines."
             ...
             (simple-indention-hooks-union-from-namespace-to
              ,namespace
             ...
             (simple-indention-hooks-add-to-hook-from-namespace
              ,namespace
              before-run-indent-func-hook
              (lambda ()
                  (setq simple-indention-increment-indent-level-function
                        (simple-indention-namespace-from
                         ,namespace increment-real-indent-level))
                  (setq simple-indention-decrement-indent-level-function
                        (simple-indention-namespace-from
             ...
             (simple-indention-hooks-add-to-hook-from-namespace
              ,namespace
              after-run-indent-func-hook
              'simple-indention-to-defaults-change-indent-function)

             (simple-indention-namespace-defun
              ,namespace
...
`${major-mode-name}-indent-some-lines-before-each-line-indent-hook'.")
              (simple-indention-hooks-from-namespace-add-hook
               ,namespace
...
`${major-mode-name}-indent-some-lines-before-each-line-indent-hook'.")
              (simple-indention-hooks-from-namespace-remove-hook
               ,namespace
               before-each-line-indent-hook
               run-indent-some-lines-before-each-line-indent-hook)
             ...
             (simple-indention-namespace-defun
              ...
              run-indent-some-lines-before-each-line-indent-hook ()
              "Run `indent-some-lines-before-each-line-indent-hook'."
              (simple-indention-hooks-run-from-namespace
               ,namespace indent-some-lines-before-each-line-indent-hook))
             ...
             (simple-indention-namespace-defun
              ...
              (let ((indent-level
                     (simple-indention-namespace-funcall
             ...
             (simple-indention-namespace-defun
              ...
              (let* ((line (simple-indention-utils-current-line))
                     (cursor 0)
                     (oi (simple-indention-namespace-var ,namespace
                                                         one-indent))
                     (step (length oi)))
             ...
             (simple-indention-namespace-defun
              ,namespace indent-line-without-run-cmd-hooks (&optional line-num)
              "Indent line with `LINE-NUM`, but don't run command hooks."
              (interactive)
              (when line-num (goto-line line-num))
              (simple-indention-rules-indent-line-with-sorted-rules
               (eval (simple-indention-namespace-from ,namespace
                                                      indention-rules))
               :each-line-before-indent-hook
               (simple-indention-namespace-from ,namespace
                                                each-line-before-indent-hook)
...
`BEG` and `END` are numbers of lines."
              ...
              (unless (= (- end beg) 0)
                  ...
                  (simple-indention-namespace-funcall
                   ,namespace indent-lines-without-run-cmd-hooks (1+ beg) end)))
...
${major-mode-name}")
             ...
             (simple-indention-namespace-defun
              ...
              (interactive "r")
              (simple-indention-namespace-funcall ,namespace indent-lines
             ...
             (simple-indention-namespace-defun
              ...
              (save-excursion
                  (when (simple-indention-utils-previous-line)
                      (simple-indention-namespace-funcall ,namespace
                                                          indent-region
                                                          (point-at-bol)
                                                          (point-at-eol))))
              (simple-indention-namespace-funcall ,namespace indent-line)
              (forward-line)
              (simple-indention-namespace-funcall ,namespace indent-line)))))


(defun simple-indention-to-defaults-change-indent-function ()
    "Set to nil `simple-indention' indent level functions."
    (setq simple-indention-increment-indent-level-function nil)
    (setq simple-indention-decrement-indent-level-function nil))


(defun simple-indention-increment-indent-level (&optional n)
    "Increment indent level on `N`.
`N` defaults to 1."
    (interactive)
    (setq n (or n 1))
    (--dotimes n (funcall simple-indention-increment-indent-level-function)))


(defun simple-indention-decrement-indent-level (&optional n)
    "Decrement indent level on `N` (deindent current line).
`N` defaults to 1."
    (interactive)
    (setq n (or n 1))
    (--dotimes n (funcall simple-indention-decrement-indent-level-function)))


(provide 'simple-indention)

;;; simple-indention.el ends here
